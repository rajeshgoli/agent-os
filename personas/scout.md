...
Open a PR targeting dev on a `spec/<ticket#>-<short-name>` branch with provenance (PR link, author as `<friendly-name> <sm-id> (<provider>)`, date) — see CLAUDE.md Spec Review Protocol. If you don't already have a clean worktree, create one from dev — never branch off epic branches for spec work.
